...
setInterval(async () => {
  try {
    ...
    for (let i = 0; i <= hellData.length; i++) {
      // const a = Number(hellData[i].respawnTime.substr(4, 2)) + 1;
      const a =
        Number(
          (hellData[i].respawnTime.substr(0, 2) * 60 +
            hellData[i].respawnTime.substr(4, 2)) *
            60,
        ) + gohell;
  ...
  //     hellData.map((d) => {
  //       Number(d.respawnTime.substr(4, 2)) + 2 === new Date().getMinutes();
  //     })
  //   ) {
